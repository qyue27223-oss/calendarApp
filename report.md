...
- `DayView`：
  ...
  - 订阅事件卡片（`SubscriptionEventItem`）通过左侧蓝色竖线（4dp 宽度）标识区分：
    - 天气事件卡片（`WeatherEventCard`）：显示当前温度、天气状况、空气质量，支持展开查看15日预报；
...
- `SubscriptionEventItem`：
  - 订阅事件展示组件，根据订阅类型渲染不同的卡片：
    - `WeatherEventCard`：天气信息卡片，显示当前温度、天气状况、空气质量，支持展开查看15日预报列表；
...
- **农历计算工具**：
  ...
  - 支持公历转农历（占位实现，建议替换为完整算法或第三方库）；
  - 实现天干地支计算（年、月、日），返回格式化的干支字符串；
  - 实现生肖计算，根据年份返回对应生肖；
  - 实现节气判断（简化版本，基于固定日期），支持24节气识别；
...
3. **网络订阅架构**：实现了灵活的订阅管理系统，支持天气和黄历订阅，简化了订阅流程（仅支持预定义服务），为扩展更多订阅类型提供了良好的架构基础
4. **农历计算与显示**：集成第三方库 `com.github.XhinLiang:LunarCalendar` 实现完整的农历计算，在月/周/日视图中准确显示农历日期、天干地支、生肖、节气、节日等传统文化信息
5. **现代化 UI/UX**：采用 Material3 设计规范，提供流畅的动画效果和优雅的视觉设计，订阅事件通过独特的卡片样式区分展示，月视图和周视图显示选中日期的日程列表，事件标记清晰可见
6. **增强的提醒功能**：支持提前提醒（五分钟、十五分钟、三十分钟、一小时）和响铃提醒（已完全实现，支持声音、震动和灯光控制），默认5分钟提醒，提供快速选择选项，满足不同场景的提醒需求。每个重复事件都有独立的提醒设置，确保提醒的准确性
7. **响应式架构**：利用 Kotlin Flow 和 Compose 实现自动 UI 更新，提升开发效率和用户体验
8. **一致性保证**：数据库与系统提醒状态保持严格一致，避免数据不一致问题
9. **代码质量与架构优化**：遵循最佳实践，提取共享组件，统一时间格式化器，减少代码重复，提升可维护性和可扩展性
